...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{CATALYSTX_EXAMPLE_WIKI_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{CATALYSTX_EXAMPLE_WIKI_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require CatalystX::ProtoWiki;

    CatalystX::ProtoWiki->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
catalystx_example_wiki_server.pl - Catalyst Testserver

=head1 SYNOPSIS

catalystx_example_wiki_server.pl [options]
