...
use CatalystX::ProtoWiki::Auth::Schema;
...
my $role = undef;
my $email = undef;
my $schema = CatalystX::ProtoWiki::Auth::Schema->connect("dbi:SQLite:$Bin/../db/auth.db");
...
catalystx_example_wiki_auth_admin.pl - Sets the username and password for the generated authentication database

=head1 SYNOPSIS

catalystx_example_wiki_auth_admin.pl -user username -passwd password [-role role]
